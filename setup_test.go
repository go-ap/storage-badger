...
func withOpenRoot(t *testing.T, r *repo) *repo {
	var err error
	r.path, err = Path(Config{Path: r.path})
	if err != nil {
		t.Errorf("unable to get full path for db: %s", err)
		return r
	}
	opts := badger.DefaultOptions(r.path)
	opts.Logger = logger{logFn: emptyLogFn, errFn: t.Logf}
	r.root, err = badger.Open(opts)
	if err != nil {
...
func withActivitiesToCollections(activities vocab.ItemCollection) initFn {
	return func(t *testing.T, r *repo) *repo {
		...
		if err := r.AddTo(collectionIRI, activities...); err != nil {
			t.Errorf("unble to save activities to collection %s: %s", collectionIRI, err)
		}
		return r
	}
}
